...
import com.dabee.ex92naversearchapi.databinding.RecyclerItemBinding
...
class NaverSearchAdapter constructor(var context: Context, var items:MutableList<NaverSearchItem>) : RecyclerView.Adapter<NaverSearchAdapter.VH>() {
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        Glide.with(context).load(items[position].image).error(R.drawable.ic_launcher_background).into(holder.binding.iv)

        holder.binding.root.setOnClickListener{
            context.startActivities(arrayOf(Intent(Intent.ACTION_VIEW,Uri.parse(items[position].link))))
        }
